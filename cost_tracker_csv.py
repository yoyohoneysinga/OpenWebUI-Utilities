...
class Config:
    DATA_DIR = "data"
    COST_FILE = os.path.join(DATA_DIR, "costs.csv")
    CACHE_DIR = os.path.join(DATA_DIR, ".cache")
    CACHE_TTL = 432000  # Cache TTL for 5 days
    CACHE_MAXSIZE = 16
    DECIMALS = "0.00000001"
...
class UserCostManager:
    ...
    def _ensure_cost_file_exists(self):
        if not os.path.exists(self.cost_file_path):
            with open(
                self.cost_file_path, "w", newline="", encoding="UTF-8"
            ) as cost_file:
    ...
    def update_user_cost(
        ...
        with open(self.cost_file_path, "a", newline="", encoding="UTF-8") as cost_file:
            writer = csv.writer(cost_file)
            writer.writerow(
                [
                    user_email,
...
class Filter:
    ...
    def __init__(self):
        ...
        self.user_cost_manager = UserCostManager(Config.COST_FILE)
